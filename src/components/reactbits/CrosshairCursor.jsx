...
const Crosshair = ({ color = 'white', containerRef = null }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = ev => {
      mouseRef.x = getMousePos(ev, containerRef?.current).x;
      mouseRef.y = getMousePos(ev, containerRef?.current).y;
      ...
      if (containerRef?.current) {
        ...
        gsap.to([lineHorizontalRef.current, lineVerticalRef.current], { opacity: outside ? 0 : 1 });
      }
    };
    ...
    const render = () => {
      renderedStyles.tx.current = mouseRef.x;
      renderedStyles.ty.current = mouseRef.y;
    ...
    const onFirstMove = () => {
      renderedStyles.tx.previous = renderedStyles.tx.current = mouseRef.x;
      renderedStyles.ty.previous = renderedStyles.ty.current = mouseRef.y;
    ...
    const leave = () => tl.progress(1).pause();
  ...
  return (
    <div
      ref={cursorRef}
      className="cursor"
      style={{ position: containerRef ? 'absolute' : 'fixed', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 10000 }}
    >
      ...
      <div ref={lineHorizontalRef} style={{ position: 'absolute', width: '100%', height: '1px', background: color, pointerEvents: 'none', transform: 'translateY(50%)', opacity: 0 }} />
      <div ref={lineVerticalRef} style={{ position: 'absolute', height: '100%', width: '1px', background: color, pointerEvents: 'none', transform: 'translateX(50%)', opacity: 0 }} />
    </div>
  );
};
